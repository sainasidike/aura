...
日运要具体到场景和建议。`;
    ...
    const wordCount = period === 'yearly' ? '200-350' : period === 'monthly' ? '150-250' : '80-150';
    const totalHint = period === 'yearly' ? '总字数1200-1800' : period === 'monthly' ? '总字数900-1400' : '总字数500-800';
...
## 输出格式
5个标记分隔，每段${wordCount}字，首句引用具体排盘数据：
---LOVE--- ---CAREER--- ---HEALTH--- ---STUDY--- ---SOCIAL---
标记：📌核心判断 💡积极建议 ⚠️注意事项
${totalHint}。禁止套话（如"劳逸结合""保持好心情"），禁止编造数据，每段必须引用上方排盘数据中的真实数据作为依据。`;
