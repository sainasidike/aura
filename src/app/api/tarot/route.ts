...
export async function POST(request: NextRequest) {
  try {
    ...
    const { question, cards, spread } = body as {
      question: string;
...
- **基本含义**：${desc}`;
    ...
    const systemPrompt = `你是一位经验丰富且极具洞察力的塔罗牌解读师。你深谙韦特塔罗牌的象征意义，善于将牌面信息与求问者的具体问题结合，给出准确、深入且富有启发性的解读。
...
请使用中文回答，保持专业性的同时通俗易懂。每张牌的解读要结合问题深入分析，不要泛泛而谈。`;
