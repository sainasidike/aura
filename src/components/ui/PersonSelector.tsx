...
export default function PersonSelector({ persons, activeId, onSelect, onAdd }: Props) {
  return (
    <div className="no-scrollbar flex gap-2 overflow-x-auto pb-1 mb-5">
      {persons.map((p) => {
        ...
        return (
          <motion.button
            ...
            style={{
              background: isActive
                ? "linear-gradient(135deg, rgba(123,108,184,0.12), rgba(58,191,182,0.06))"
                : "var(--bg-base)",
              border: `1px solid ${isActive ? "var(--accent-primary)" : "var(--border-subtle)"}`,
              color: isActive ? "var(--accent-primary)" : "var(--text-primary)",
              boxShadow: isActive ? "0 2px 12px rgba(123,108,184,0.15)" : "var(--shadow-sm)",
              fontWeight: isActive ? 600 : 400,
            }}
            whileTap={{ scale: 0.95 }}
          >
            <div
              className="flex h-6 w-6 items-center justify-center rounded-full text-[0.65rem] font-semibold text-white"
